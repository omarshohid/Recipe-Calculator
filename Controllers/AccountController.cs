        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    using (RecipeCalculatorEntities rc = new RecipeCalculatorEntities())
                    {
                        try
                        {
                            tblRecipeUser recipeUser = new tblRecipeUser();
                          ...
                          //  DateTime processDate = dayProcessFactory.GetAll().OrderByDescending(a => a.process_date).Select(a => a.DimDate.Date).FirstOrDefault() ?? DateTime.Now;
                            recipeUser.UserId = user.Id;                            
                            recipeUser.is_loggedIn = 1;
                            recipeUser.last_logged_time = DateTime.Now;
                          
                            // SessionManger.SetBrokerOfLoggedInUser(Session, brokerFactory.FindBy(b => b.short_name == model.BrokerName).FirstOrDefault());

                       
                            rc.tblRecipeUsers.Attach(recipeUser);
                            var entry = rc.Entry(recipeUser);
                            entry.State = EntityState.Modified;
                            rc.SaveChanges();
                            SessionManger.SetLoggedInTime(Session, DateTime.Now);

                            SessionManger.SetLoggedInUser(Session, model.UserName);
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                else 
                {
                    ...
                    RecipeCalculatorEntities recipeCal = new RecipeCalculatorEntities();
                    AspNetUser anu = new AspNetUser();

                    //AspNetUser userId = recipeCal.AspNetRoles.Select(a => a.Name == model.UserName).FirstOrDefault();
                    AspNetUser searchUser = recipeCal.AspNetUsers.Where(a => a.UserName == model.UserName).FirstOrDefault();
                    ...
                    {                       
                        ...
                        using ( RecipeCalculatorEntities rc = new RecipeCalculatorEntities())
                        {
                            try
                            {
                                ...
                                recipeUser.UserId = searchUser.Id;
                               // recipeUser.is_admin = 0;
                                //recipeUser.is_loggedIn = null;
                                //recipeUser.last_logged_time = null;
                                //recipeUser.approve_by = null;
                                //recipeUser.approve_date = null;
                                rc.tblRecipeUsers.Add(recipeUser);
                                rc.SaveChanges();
                    ...
                    {                     
                        return Json(new { success = false, errorMessage = "Email has already taken"}, JsonRequestBehavior.AllowGet);
